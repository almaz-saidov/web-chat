...
class SignatureService(DatabaseService[SignatureTemplateRepository]):
    async def create_template(
        ...
        return await self._repository.create(signature_template_create_data=signature_template_create_data)

    async def verify_signature(self, user_id: uuid.UUID, signature_sample: SignatureSampleSchema) -> bool:
        signature_template = await self._repository.get_by_user_id(user_id=user_id)
        if not signature_template:
            return False

        current_template_data = self._normalize_sample_data(signature_sample=signature_sample)
    ...
    def _build_template(self, signature_samples: list[SignatureSampleSchema]) -> SignatureTemplateDataSchema:
        if len(signature_samples) != settings.SIGNATURE_SAMPLE_COUNT:
            raise ValueError(f"Signature template requires exactly {settings.SIGNATURE_SAMPLE_COUNT} samples")

        normalized_samples = [
            self._normalize_sample_data(signature_sample=signature_sample) for signature_sample in signature_samples
        ]

        averaged_points = [
            self._average_points(points=[sample.points[point_index] for sample in normalized_samples])
        ...
        return SignatureTemplateDataSchema(
            ...
            duration_ms=self._average(values=[sample.duration_ms for sample in normalized_samples]),
            break_count=self._average(values=[sample.break_count for sample in normalized_samples]),
        )

    def _normalize_sample_data(self, signature_sample: SignatureSampleSchema) -> SignatureTemplateDataSchema:
        centered_points = self._center_and_scale_points(points=signature_sample.points)
        normalized_time_points = self._normalize_time(points=centered_points, duration_ms=signature_sample.duration_ms)
    ...
    def _center_and_scale_points(self, points: list[SignaturePointSchema]) -> list[SignaturePointSchema]:
    ...
    def _normalize_time(self, points: list[SignaturePointSchema], duration_ms: float) -> list[SignaturePointSchema]:
        return [point.model_copy(update={"time_ms": point.time_ms / duration_ms}) for point in points]
    ...
    def _resample_points(
        ...
        for point_index in range(point_count):
            ...
            while source_index < len(distances) - 1 and distances[source_index] < target_distance:
                source_index += 1
    ...
    def _get_cumulative_distances(self, points: list[SignaturePointSchema]) -> list[float]:
        distances = [0.0]
    ...
    def _smooth_points(self, points: list[SignaturePointSchema]) -> list[SignaturePointSchema]:
        smoothed_points: list[SignaturePointSchema] = []

        for point_index in range(len(points)):
            window_start = max(point_index - 1, 0)
            window_end = min(point_index + 2, len(points))
            smoothed_points.append(self._average_points(points=points[window_start:window_end]))
    ...
    def _calculate_score(
        ...
        break_count_penalty = abs(reference_template_data.break_count - current_template_data.break_count)

        return 1 / (1 + dtw_distance + duration_penalty + break_count_penalty * settings.SIGNATURE_BREAK_COUNT_WEIGHT)
    ...
    def _calculate_dtw_distance(
        ...
        for current_point in current_points:
            ...
            for reference_index, reference_point in enumerate(reference_points, start=1):
    ...
    def _calculate_point_distance(
        ...
        tilt_distance = (
            abs(reference_point.tilt_x - current_point.tilt_x) + abs(reference_point.tilt_y - current_point.tilt_y)
        ) / 180
    ...
    def _calculate_duration_penalty(self, reference_duration_ms: float, current_duration_ms: float) -> float:
        duration_ratio = abs(reference_duration_ms - current_duration_ms) / max(reference_duration_ms, 1)
        return min(duration_ratio, settings.SIGNATURE_MAX_DURATION_PENALTY_RATIO) * settings.SIGNATURE_DURATION_WEIGHT
    ...
    def _interpolate_points(
        ...
        return SignaturePointSchema(
            ...
            pressure=self._interpolate(first_point.pressure, second_point.pressure, ratio),
    ...
    def _average_points(self, points: list[SignaturePointSchema]) -> SignaturePointSchema:
        return SignaturePointSchema(
    ...
    def _get_interpolation_ratio(self, target_distance: float, previous_distance: float, next_distance: float) -> float:
        if next_distance == previous_distance:
            return 0

        return (target_distance - previous_distance) / (next_distance - previous_distance)

    def _interpolate(self, first_value: float, second_value: float, ratio: float) -> float:
