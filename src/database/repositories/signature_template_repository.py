...
class SignatureTemplateRepository(BaseDatabaseRepository):
    async def create(
        self, signature_template_create_data: SignatureTemplateCreateSchema
    ) -> SignatureTemplateSchema:
        query = (
            insert(SignatureTemplate)
            .values(**signature_template_create_data.model_dump())
            .returning(SignatureTemplate)
        )
    ...
    async def get_by_user_id(
        ...
        return (
            SignatureTemplateSchema.model_validate(signature_template)
