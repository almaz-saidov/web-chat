...
async def test_create_token_deletes_existing_token_for_user_before_create() -> None:
    ...
    create_data = RefreshTokenCreateSchema(user_id=user_id, expires_at=datetime.now(timezone.utc))
    created_token = make_refresh_token_schema(user_id=user_id, expires_at=create_data.expires_at)
    repository = make_repository()
