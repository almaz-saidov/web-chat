...
def assert_access_token_response(
    response: Response, failure_message_prefix: str
) -> None:
    response_data = response.json()

    assert isinstance(response_data, dict), (
        f"{failure_message_prefix} response must be a JSON object"
    )
    assert "access_token" in response_data, (
        f"{failure_message_prefix} response must contain access token"
    )
    assert response_data.get("token_type") == "Bearer", (
        f"{failure_message_prefix} response must use Bearer token type"
    )
    assert isinstance(response_data["access_token"], str), (
        f"{failure_message_prefix} access token must be a string"
    )
    assert response_data["access_token"], (
        f"{failure_message_prefix} access token must not be empty"
    )
...
def assert_refresh_cookie_set(response: Response) -> None:
    ...
    assert "refresh_token=" in set_cookie_header, (
        "Refresh token cookie must be present in Set-Cookie header"
    )
...
def get_refresh_cookie_headers(response: Response) -> dict[str, str]:
    ...
    assert refresh_token is not None, (
        "Login response cookies must contain refresh token"
    )
    return {"Cookie": f"refresh_token={refresh_token}"}


async def test_register_returns_201_for_valid_payload(
    ...
    assert response.status_code == status.HTTP_201_CREATED, (
        "Register endpoint must return 201 for valid payload"
    )
    response_data = response.json()
    assert isinstance(response_data, dict), "Register response must be a JSON object"
    assert response_data.get("username") == payload["username"], (
        "Register response must contain created username"
    )
    assert "id" in response_data, "Register response must contain user id"
    assert "created_at" in response_data, (
        "Register response must contain user creation timestamp"
    )
    assert "password_hash" not in response_data, (
        "Register response must not expose password hash"
    )


async def test_register_returns_409_for_duplicate_username(
    async_client: AsyncClient,
) -> None:
    payload = make_register_payload()
    ...
    assert response.status_code == status.HTTP_409_CONFLICT, (
        "Register endpoint must reject duplicate username"
    )
    assert_response_detail(
        response=response, expected_detail="The user with this username already exists"
    )


async def test_register_returns_422_for_invalid_payload(
    async_client: AsyncClient,
) -> None:
    response = await async_client.post(
        "/api/auth/register",
    ...
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT, (
        "Register endpoint must validate payload"
    )
    response_data = response.json()
    assert isinstance(response_data, dict), (
...
async def test_register_returns_422_for_wrong_signature_sample_count(
    async_client: AsyncClient,
) -> None:
    payload = make_register_payload()
...
async def test_register_returns_422_for_short_signature_sample(
    async_client: AsyncClient,
) -> None:
    payload = make_register_payload()
...
async def test_login_returns_200_and_refresh_cookie_for_valid_credentials(
    async_client: AsyncClient,
) -> None:
    payload = make_register_payload()
    await register_user(async_client=async_client, payload=payload)
...
async def test_login_returns_422_without_signature_sample(
    ...
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT, (
        "Login endpoint must require signature sample"
    )
    response_data = response.json()
    assert isinstance(response_data, dict), (
        "Validation error response must be a JSON object"
...
async def test_login_returns_422_for_short_signature_sample(
    async_client: AsyncClient,
) -> None:
    response = await async_client.post(
        "/api/auth/login",
...
async def test_login_returns_401_for_wrong_password(async_client: AsyncClient) -> None:
    ...
    assert response.status_code == status.HTTP_401_UNAUTHORIZED, (
        "Login endpoint must reject wrong password"
    )
    assert_response_detail(
        response=response, expected_detail="Wrong username or password"
    )
...
async def test_login_returns_401_for_wrong_signature(async_client: AsyncClient) -> None:
    ...
    assert_response_detail(
        response=response, expected_detail="Signature verification failed"
    )


async def test_refresh_returns_200_for_valid_refresh_cookie(
    async_client: AsyncClient,
) -> None:
    payload = make_register_payload()
    await register_user(async_client=async_client, payload=payload)
    ...
    response = await async_client.post(
        "/api/auth/refresh", headers=get_refresh_cookie_headers(response=login_response)
    )

    assert response.status_code == status.HTTP_200_OK, (
        "Refresh endpoint must return 200 with valid refresh cookie"
    )
    assert_access_token_response(response=response, failure_message_prefix="Refresh")
    assert_refresh_cookie_set(response=response)


async def test_refresh_returns_401_without_refresh_cookie(
    async_client: AsyncClient,
) -> None:
    response = await async_client.post("/api/auth/refresh")

    assert response.status_code == status.HTTP_401_UNAUTHORIZED, (
        "Refresh endpoint must require refresh token cookie"
    )
    assert_response_detail(
        response=response, expected_detail="Refresh token cookie is missing"
    )


async def test_logout_returns_200_and_deletes_refresh_cookie(
    async_client: AsyncClient,
) -> None:
    payload = make_register_payload()
    await register_user(async_client=async_client, payload=payload)
    ...
    response = await async_client.post(
        "/api/auth/logout", headers=get_refresh_cookie_headers(response=login_response)
    )

    assert response.status_code == status.HTTP_200_OK, (
        "Logout endpoint must return 200 for valid refresh cookie"
    )
    set_cookie_header = response.headers.get("set-cookie")
    assert set_cookie_header is not None, (
        "Logout response must include Set-Cookie header"
    )
    assert "refresh_token=" in set_cookie_header, (
        "Logout response must target refresh token cookie"
    )
    assert "Max-Age=0" in set_cookie_header, (
        "Logout response must expire refresh token cookie immediately"
    )
